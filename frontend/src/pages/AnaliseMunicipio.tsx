// src/pages/AnaliseMunicipio.tsx â€” AnÃ¡lise Brasil 360 para qualquer municÃ­pio
...
// â”€â”€ MunicÃ­pios brasileiros (sample â€” busca por nome) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Em produÃ§Ã£o, buscaria da API IBGE
const ESTADOS = [
  "AC","AL","AP","AM","BA","CE","DF","ES","GO","MA","MT","MS","MG",
  "PA","PB","PR","PE","PI","RJ","RN","RS","RO","RR","SC","SP","SE","TO"
];

// â”€â”€ Pesos por indicador (Portaria 3.493/2024) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ DescriÃ§Ãµes dos indicadores â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const DESC_C: Record<string,string> = {
  c1:"Acesso Avaliado", c2:"PrÃ©-natal Adequado", c3:"Rastreio Colo Ãštero",
  c4:"Rastreio Mama", c5:"HAS Controlada", c6:"Puericultura", c7:"SaÃºde Bucal Gestante"
};
const DESC_B: Record<string,string> = {
  b1:"1Âª Consulta OdontolÃ³gica", b2:"ConclusÃ£o Tratamento", b3:"UrgÃªncia Odonto",
  b4:"Gestante Odonto", b5:"PrevenÃ§Ã£o Coletiva", b6:"EscovaÃ§Ã£o Supervisionada"
};
const DESC_M: Record<string,string> = { m1:"SaÃºde Mental", m2:"ReabilitaÃ§Ã£o" };

// â”€â”€ CÃ¡lculo de score â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function classificar(pts: number) {
  if (pts >= 75) return { label: "Ã“TIMO", cor: "#22c55e", bg: "#14532d" };
...
// â”€â”€ AÃ§Ãµes recomendadas por indicador â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const ACOES: Record<string, string> = {
  C1: "Registrar retorno de 30 dias no PEC apÃ³s cada consulta. Tipo de atendimento correto.",
  C2: "HbA1c + VDRL na 1Âª consulta prÃ©-natal. LanÃ§ar resultado no PEC com tipo correto.",
  C3: "Busca ativa mulheres 25-64 anos sem citopatolÃ³gico. Dia D de coleta.",
  C4: "Busca ativa mulheres 50-69 anos. Solicitar mamografia e lanÃ§ar resultado no PEC.",
  C5: "TÃ©cnico de enfermagem lanÃ§a PA em TODA consulta de hipertenso. Meta: PA <140/90.",
  C6: "Agenda de puericultura 2x/semana. Busca ativa crianÃ§as <2 anos via ACS.",
  C7: "Consulta odontolÃ³gica da gestante no prÃ©-natal. Integrar agendas odonto + prÃ©-natal.",
  B1: "Agenda de primeira consulta odontolÃ³gica. Busca ativa de pacientes sem consulta.",
  B2: "Finalizar tratamentos em andamento no PEC. Registrar conclusÃ£o corretamente.",
  B3: "Protocolo de urgÃªncia odonto disponÃ­vel. Registro de atendimentos de urgÃªncia.",
  B4: "Gestante deve ter consulta odonto no prÃ©-natal. Integrar com equipe eSF.",
  B5: "Atividade coletiva de escovaÃ§Ã£o. Registrar no PEC como atividade coletiva.",
  B6: "SupervisÃ£o de escovaÃ§Ã£o nas escolas. Registrar no PEC.",
  M1: "Casos de saÃºde mental identificados e acompanhados pelo eMulti. LanÃ§ar no PEC.",
  M2: "Casos de reabilitaÃ§Ã£o identificados. Atendimentos lanÃ§ados no PEC com INE correto.",
};
  ...
  function imprimir() {
    ...
    const rows = resultados.map(r => {
      ...
      return `<tr>
        ...
        <td>${r.total >= 75 ? "â€”" : "+" + (75 - r.total).toFixed(1) + " pts"}</td>
        <td>${topGap ? topGap.ind + " (" + topGap.atual + "%)" : "â€”"}</td>
        <td>${topGap ? ACOES[topGap.ind] || "â€”" : "Manter protocolo"}</td>
      </tr>`;
    }).join("");
    w.document.write(`<!doctype html><html><head><meta charset="utf-8">
    <title>AnÃ¡lise Brasil 360 â€” ${municipio.nome}/${municipio.uf}</title>
    <style>
    ...
    <h1>AnÃ¡lise Brasil 360 â€” Componente Qualidade</h1>
    <h2>${municipio.nome}/${municipio.uf} Â· IBGE: ${municipio.codigo || "â€”"} Â· Quadrimestre: ${quadrimestre} Â· Portaria GM/MS 3.493/2024</h2>
    <div style="margin-bottom:16px">
      <span class="kpi"><b>${mediaGeral}</b>MÃ©dia Geral</span>
      <span class="kpi"><b style="color:#22c55e">${otimo}</b>Ã“TIMO (â‰¥75)</span>
      <span class="kpi"><b style="color:#f59e0b">${bom}</b>BOM (60â€“74)</span>
      <span class="kpi"><b style="color:#ef4444">${risco}</b>Risco (&lt;60)</span>
      <span class="kpi"><b>${equipes.length}</b>Equipes</span>
    </div>
    <table><thead><tr>
      <th>Equipe</th><th>Tipo</th><th>Score</th><th>ClassificaÃ§Ã£o</th>
      <th>Falta p/ Ã“TIMO</th><th>Maior Gap</th><th>AÃ§Ã£o PrioritÃ¡ria</th>
    </tr></thead><tbody>${rows}</tbody></table>
    <p style="font-size:10px;color:#888;margin-top:20px">
      Gerado pelo ERSUS360 Â· ${new Date().toLocaleString("pt-BR")} Â· Scores informados manualmente via e-Gestor AB
    </p>
  ...
  return (
    <div style={s.page}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 17 }}>AnÃ¡lise Brasil 360 â€” Qualquer MunicÃ­pio</span>
            </div>
            <div style={{ fontSize: 12, color: "#6b7280" }}>Portaria GM/MS 3.493/2024 Â· Componente Qualidade Â· eSF, eSB, eMulti, eRibeirinha</div>
          </div>
          {etapa === "analise" && (
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #1e293b", padding: "0 24px" }}>
        {[
          { key: "municipio", label: "1. MunicÃ­pio" },
          { key: "equipes",   label: "2. Equipes & Scores" },
          { key: "analise",   label: "3. AnÃ¡lise" },
        ].map((t, i) => (
      ...
      <div style={s.body}>
        ...
        {/* ETAPA 1 â€” MunicÃ­pio */}
        {etapa === "municipio" && (
          <div style={{ maxWidth: 560 }}>
            <div style={s.card}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#111827", marginBottom: 16 }}>IdentificaÃ§Ã£o do MunicÃ­pio</div>
              ...
              <div style={{ ...s.grid2, marginBottom: 14 }}>
                ...
                <div>
                  <label style={s.label}>CÃ³digo IBGE</label>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={s.label}>Nome do MunicÃ­pio</label>
                <input style={s.input} placeholder="Ex: ApuÃ­" value={municipio.nome}
                  onChange={e => setMunicipio(p => ({ ...p, nome: e.target.value }))} />
              </div>

              <div style={{ marginBottom: 20 }}>
                <label style={s.label}>Quadrimestre de ReferÃªncia</label>
              ...
              <div style={{ background: "#f4f6f8", borderRadius: 8, padding: 14, marginBottom: 20, border: "1px solid #1e3a5f" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#3b82f6", marginBottom: 8 }}>ðŸ“‹ Como usar</div>
                <ol style={{ fontSize: 12, color: "#6b7280", paddingLeft: 16, margin: 0, lineHeight: 1.8 }}>
                  <li>Acesse o <strong style={{ color: "#111827" }}>e-Gestor AB</strong> â†’ SIAPS â†’ Componente Qualidade</li>
                  <li>Selecione o municÃ­pio e o quadrimestre</li>
                  <li>Anote o score de cada indicador de cada equipe</li>
                  <li>Preencha aqui e gere a anÃ¡lise completa</li>
                </ol>
              </div>
        ...
        {etapa === "equipes" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15 }}>{municipio.nome}/{municipio.uf} â€” {quadrimestre}</div>
            ...
            {equipes.map((eq, idx) => {
              ...
              return (
                <div key={eq.id} style={{ ...s.card, border: `1px solid ${cl.cor}44` }}>
                  ...
                  {aberto && (
                    <div>
                      {/* VÃ­nculo e Acompanhamento + ImplantaÃ§Ã£o */}
                      <div style={{ ...s.grid2, marginBottom: 16 }}>
                        <IndSlider id={eq.id} campo="va" label="VÃ­nculo e Acompanhamento (%)" valor={eq.va} />
                        <IndSlider id={eq.id} campo="impl" label="ImplantaÃ§Ã£o (%)" valor={eq.impl} />
                      </div>

                      <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>
                        Componente Qualidade â€” {eq.tipo === "eSB" ? "Grupo B" : eq.tipo === "eMulti" ? "Grupo M" : "Grupo C"}
                      </div>

                      {/* eSF / eAP / eRibeirinha â†’ Grupo C */}
                      {(eq.tipo === "eSF" || eq.tipo === "eAP" || eq.tipo === "eRibeirinha") && (
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 12 }}>
                          {indsC.map(k => <IndSlider key={k} id={eq.id} campo={k} label={`${k.toUpperCase()} â€” ${DESC_C[k]}`} valor={eq[k]} />)}
                        </div>
                      )}
                      ...
                      {eq.tipo === "eSB" && (
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 12 }}>
                          {indsB.map(k => <IndSlider key={k} id={eq.id} campo={k} label={`${k.toUpperCase()} â€” ${DESC_B[k]}`} valor={eq[k]} />)}
                        </div>
                      )}
                      ...
                      {eq.tipo === "eMulti" && (
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                          {indsM.map(k => <IndSlider key={k} id={eq.id} campo={k} label={`${k.toUpperCase()} â€” ${DESC_M[k]}`} valor={eq[k]} />)}
                        </div>
                      )}
                    </div>
                  )}
        ...
        {etapa === "analise" && (
          <div ref={printRef}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 12, marginBottom: 20 }}>
              {[
                { label: "MÃ©dia Geral", valor: mediaGeral, cor: classificar(mediaGeral).cor },
                { label: "Ã“TIMO â‰¥75", valor: otimo, cor: "#22c55e" },
                { label: "BOM 60â€“74", valor: bom, cor: "#f59e0b" },
            ...
            <div style={{ ...s.card, overflow: "auto" }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Resultado por Equipe â€” {municipio.nome}/{municipio.uf} Â· {quadrimestre}</div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 700 }}>
                <thead>
                  <tr style={{ background: "#f4f6f8" }}>
                    {["Equipe","Tipo","Score","ClassificaÃ§Ã£o","Falta p/ Ã“TIMO","Maior Gap","AÃ§Ã£o PrioritÃ¡ria"].map(h => (
                    ...
                    return (
                      <tr key={r.eq.id} style={{ borderBottom: "1px solid #1e293b" }}>
                        ...
                        <td style={{ padding: "9px 10px", color: r.total >= 75 ? "#22c55e" : "#ef4444", fontWeight: 600 }}>
                          {r.total >= 75 ? "âœ… Atingido" : `+${(75 - r.total).toFixed(1)} pts`}
                        </td>
                        <td style={{ padding: "9px 10px" }}>
                          {gap ? <span style={{ color: "#f59e0b", fontWeight: 600 }}>{gap.ind} ({gap.atual}%)</span> : <span style={{ color: "#22c55e" }}>â€”</span>}
                        </td>
                        <td style={{ padding: "9px 10px", fontSize: 12, color: "#6b7280", maxWidth: 260 }}>
                          {gap ? ACOES[gap.ind] || "â€”" : "Manter protocolo atual"}
                        </td>
                      </tr>
                    );
                  })}
            ...
            {/* AnÃ¡lise por equipe â€” gaps detalhados */}
            ...
            {resultados.filter(r => r.total >= 75).length > 0 && (
              <div style={{ ...s.card, borderLeft: "4px solid #22c55e" }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                  ...
                  <span style={{ fontWeight: 700, color: "#22c55e" }}>Equipes jÃ¡ em Ã“TIMO â€” Manter protocolo</span>
                </div>
            ...
            <div style={{ ...s.card, background: "#f4f6f8", border: "1px solid #1e3a5f", marginTop: 8 }}>
              <div style={{ fontSize: 11, color: "#6b7280" }}>
                AnÃ¡lise gerada pelo ERSUS360 Â· Scores informados manualmente via e-Gestor AB Â· Portaria GM/MS 3.493/2024
              </div>
            </div>
          </div>
        )}
