...
function AbaVeiculos({ veiculos }: { veiculos: any[] | undefined }) {
  if (!veiculos) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      {veiculos.map(v => {
...
function AbaManutencoes({ mans }: { mans: any[] | undefined }) {
  if (!mans) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      {mans.map(m => {
