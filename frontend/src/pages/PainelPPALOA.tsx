...
// ── SIOPS histórico ───────────────────────────────────────────────────────────
function AbaSIOPS({ historico }: { historico: any[] | undefined }) {
  if (!historico) return null;
  return (
    <div>
      <div style={{ marginBottom: 18 }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, color: "#1d4ed8", margin: "0 0 4px" }}>SIOPS — Recursos Próprios em Saúde</h2>
        <p style={{ fontSize: 13, color: "#6b7280", margin: 0 }}>EC 29/2000 — Municípios: mínimo 15% da receita própria em saúde</p>
      </div>
      <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, padding: "12px 16px", marginBottom: 20, display: "flex", gap: 10, alignItems: "center" }}>
        ...
        <span style={{ fontSize: 13, color: "#166534" }}>
          Apuí/AM está <strong>conforme</strong> em todos os anos do PPA. Ano 2026: <strong>17,16%</strong> (meta ≥ 15%).
        </span>
      </div>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 18 }}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Histórico de aplicação de recursos próprios (%)</div>
        <div style={{ height: 200 }}>
          <ResponsiveContainer width="100%" height="100%">
        ...
        <div style={{ marginTop: 12 }}>
          {historico.map(h => (
            <div key={h.ano} style={{ display: "inline-flex", alignItems: "center", gap: 6, marginRight: 16, fontSize: 12 }}>
...
export default function PainelPPALOA() {
  ...
  const { data: siops }    = useQuery({ queryKey: ["ppa-siops"],     queryFn: () => apiGet("/api/planejamento-orc/siops-historico") as Promise<any[]>, enabled: aba === "siops" });
  ...
  return (
    <div style={{ padding: "0 0 32px", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ padding: "0 24px" }}>
        ...
        {aba === "siops" && <AbaSIOPS historico={siops} />}
      </div>
    </div>
  );
}
