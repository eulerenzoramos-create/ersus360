...
function AbaAfastamentos({ afasts }: { afasts: any[] | undefined }) {
  if (!afasts) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
...
function AbaCAT({ cats }: { cats: any[] | undefined }) {
  if (!cats) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      {cats.map(c => {
