...
function AbaAfastamentos({ afasts }: { afasts: any[] | undefined }) {
  if (!afasts) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
...
function AbaCAT({ cats }: { cats: any[] | undefined }) {
  if (!cats) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
      {cats.map(c => {
