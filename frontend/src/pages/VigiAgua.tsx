...
function AbaSistemas({ sistemas }: { sistemas: any[] | undefined }) {
  if (!sistemas) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
      {sistemas.map(s => {
...
function AbaParametros({ params }: { params: any[] | undefined }) {
  if (!params) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 18 }}>
