...
function AbaSistemas({ sistemas }: { sistemas: any[] | undefined }) {
  if (!sistemas) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      {sistemas.map(s => {
...
function AbaParametros({ params }: { params: any[] | undefined }) {
  if (!params) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 18 }}>
