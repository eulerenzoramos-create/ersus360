...
function AbaModalidades({ mods }: { mods: any[] | undefined }) {
  if (!mods) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
      {mods.map(m => {
...
function AbaCondicoes({ conds }: { conds: any[] | undefined }) {
  if (!conds) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 18 }}>
