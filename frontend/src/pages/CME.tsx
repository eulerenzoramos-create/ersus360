...
function AbaEquipamentos({ ciclos }: { ciclos: any[] | undefined }) {
  if (!ciclos) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
      {ciclos.map(eq => {
...
function AbaRastreabilidade({ lotes }: { lotes: any[] | undefined }) {
  if (!lotes) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
      {lotes.map(l => {
