    ...
    <div style={{ padding: "0 0 60px 0", fontFamily: "Inter, system-ui, sans-serif", background: "#fff", color: "#111827" }}>
            ...
            {editandoMunicipio ? (
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                <input
                  ...
                  style={{ background: "#ffffff", border: "1px solid #d1d5db", borderRadius: 8, color: "#111827", padding: "6px 12px", fontSize: 13, width: 160, outline: "none" }}
                />
                <select value={municipioUF} onChange={e => setMunicipioUF(e.target.value)}
                  style={{ background: "#ffffff", border: "1px solid #d1d5db", borderRadius: 8, color: "#111827", padding: "6px 10px", fontSize: 13, outline: "none" }}>
                ...
                <input
                  ...
                  placeholder="IBGE"
                  style={{ background: "#ffffff", border: "1px solid #d1d5db", borderRadius: 8, color: "#111827", padding: "6px 12px", fontSize: 13, width: 90, outline: "none" }}
                />
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "#111827" }}>{municipioNome} / {municipioUF}</div>
                  {municipioIBGE && <div style={{ fontSize: 11, color: "#475569" }}>IBGE {municipioIBGE}</div>}
                </div>
                <button onClick={() => setEditandoMunicipio(true)}
                  style={{ background: "rgba(59,130,246,0.1)", color: "#1565c0", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 8, padding: "4px 10px", fontSize: 11, cursor: "pointer" }}>
                  ✏️
                </button>
              </div>
            )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 0, background: "#0a1020", borderBottom: "1px solid #e4e7ec" }}>
        {[
          ...
          { label: "Sprint encerra", value: `${diasRestantes}d ${horasRestantes}h`, sub: periodoAtual.desc, color: "#1565c0", bg: "rgba(59,130,246,0.07)" },
        ].map((s, i, arr) => (
      ...
      <div style={{ padding: "24px 28px" }}>
        ...
        {aba === "cvat" && (() => {
          ...
          return (
            <div>
              {/* Header CVAT */}
              <div style={{ background: "#f0f7ff", border: "1px solid #dbeafe", borderRadius: 10, padding: "14px 18px", marginBottom: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <div style={{ background: "#f8fafc", border: "1px solid #e4e7ec", borderRadius: 8, padding: "6px 14px", fontSize: 11, color: "#1d4ed8" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                {(["variavel","equipe"] as const).map(v => (
                  <button key={v} onClick={() => setCvatVizualiz(v)} style={{
                    ...
                    border: `1px solid ${cvatVizualiz === v ? "#0078d4" : "#e4e7ec"}`,
                    background: cvatVizualiz === v ? "#dbeafe" : "transparent",
                    color: cvatVizualiz === v ? "#1d4ed8" : "#6b7280",
                  }}>
              ...
              {cvatVizualiz === "variavel" && (
                <div>
                  ...
                  <div style={{ background: "#0d1f35", border: "1px solid #1e3a5f", borderRadius: 10, padding: "14px 18px", marginBottom: 16 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#1d4ed8", marginBottom: 3 }}>{variavelAtual.label}</div>
                    <div style={{ fontSize: 11.5, color: "#6b7280", marginBottom: 8 }}>{variavelAtual.desc}</div>
                    <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                      <div><span style={{ fontSize: 22, fontWeight: 800, color: "#111827" }}>{totalMunicipal[variavelAtual.key].toLocaleString("pt-BR")}</span><span style={{ fontSize: 11, color: "#6b7280", marginLeft: 6 }}>total município</span></div>
                      {cvatVariavel !== "semCriterio" && totalMunicipal.semCriterio > 0 && (
                        <div><span style={{ fontSize: 22, fontWeight: 800, color: "#f59e0b" }}>{((totalMunicipal[variavelAtual.key] / totalMunicipal.semCriterio) * 100).toFixed(1)}%</span><span style={{ fontSize: 11, color: "#6b7280", marginLeft: 6 }}>da pop. vinculada</span></div>
                      )}
                      <div><span style={{ fontSize: 22, fontWeight: 800, color: "#10b981" }}>{EQUIPES.filter(e => e.risco !== "apurar").length}</span><span style={{ fontSize: 11, color: "#6b7280", marginLeft: 6 }}>equipes ativas</span></div>
                    </div>
                  </div>

                  {/* Tabela por equipe para a variável selecionada */}
                  <div style={{ background: "#ffffff", border: "1px solid #e4e7ec", borderRadius: 10, overflow: "hidden" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
                      <thead>
                        <tr style={{ background: "#f8fafc" }}>
                          <th style={{ padding: "10px 14px", textAlign: "left", color: "#6b7280", fontWeight: 600, fontSize: 11 }}>Equipe</th>
                          <th style={{ padding: "10px 14px", textAlign: "left", color: "#6b7280", fontWeight: 600, fontSize: 11 }}>Tipo</th>
                          <th style={{ padding: "10px 14px", textAlign: "right", color: "#6b7280", fontWeight: 600, fontSize: 11 }}>Pop. Vinculada Total</th>
                          <th style={{ padding: "10px 14px", textAlign: "right", color: "#6b7280", fontWeight: 600, fontSize: 11 }}>{variavelAtual.label}</th>
                          {cvatVariavel !== "semCriterio" && <th style={{ padding: "10px 14px", textAlign: "right", color: "#6b7280", fontWeight: 600, fontSize: 11 }}>% da equipe</th>}
                          <th style={{ padding: "10px 14px", textAlign: "left", color: "#6b7280", fontWeight: 600, fontSize: 11 }}>Distribuição</th>
                        </tr>
                      </thead>
                      <tbody>
                        {nomes.map((eq, i) => {
                          ...
                          return (
                            <tr key={eq} style={{ borderBottom: "1px solid #f1f5f9", background: isCritico ? "rgba(239,68,68,0.04)" : isApurar ? "rgba(107,114,128,0.04)" : i % 2 === 0 ? "transparent" : "#f9fafb" }}>
                              <td style={{ padding: "10px 14px", color: isCritico ? "#ef4444" : isApurar ? "#6b7280" : "#111827", fontWeight: 700 }}>
                                {isCritico ? "🚨 " : isApurar ? "🔍 " : ""}{eq}
                              </td>
                              <td style={{ padding: "10px 14px" }}>
                                <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, background: isApurar ? "#e4e7ec" : diag?.tipo === "eRibeirinha" ? "#dbeafe" : "#14532d", color: isApurar ? "#6b7280" : diag?.tipo === "eRibeirinha" ? "#1d4ed8" : "#bbf7d0", fontWeight: 700 }}>
                                  {isApurar ? "—" : diag?.tipo || "eSF"}
                                </span>
                              </td>
                              <td style={{ padding: "10px 14px", textAlign: "right", color: isApurar ? "#6b7280" : "#6b7280", fontVariantNumeric: "tabular-nums", fontStyle: isApurar ? "italic" : "normal" }}>
                                {isApurar ? "—" : pop.toLocaleString("pt-BR")}
                              </td>
                              <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: isCritico ? "#ef4444" : isApurar ? "#6b7280" : "#111827", fontVariantNumeric: "tabular-nums", fontStyle: isApurar ? "italic" : "normal" }}>
                              ...
                              <td style={{ padding: "10px 14px", minWidth: 120 }}>
                                {isApurar
                                  ...
                                  : <div style={{ background: "#f8fafc", borderRadius: 4, height: 8, overflow: "hidden" }}>
                        ...
                        <tr style={{ background: "#f8fafc", borderTop: "2px solid #334155" }}>
                          <td colSpan={2} style={{ padding: "10px 14px", fontWeight: 800, color: "#111827", fontSize: 12 }}>TOTAL MUNICÍPIO</td>
                          <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 800, color: "#111827", fontVariantNumeric: "tabular-nums" }}>{totalMunicipal.semCriterio.toLocaleString("pt-BR")}</td>
                ...
                <div>
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5, minWidth: 900 }}>
                      <thead>
                        <tr style={{ background: "#f8fafc" }}>
                          <th style={{ padding: "10px 12px", textAlign: "left", color: "#6b7280", fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5, position: "sticky", left: 0, background: "#f8fafc", zIndex: 1 }}>Variável</th>
                          {nomes.map(eq => {
                            const st = DIAGNOSTICO.find(d=>d.nome===eq)?.cnesStatus;
                            return (
                              <th key={eq} style={{ padding: "10px 10px", textAlign: "right", color: st === "expirado" ? "#ef4444" : st === "apurar" ? "#6b7280" : "#6b7280", fontWeight: 700, fontSize: 10, whiteSpace: "nowrap" }}>
                      ...
                      <tbody>
                        {CVAT_VARIAVEIS.map((v, i) => (
                          <tr key={v.key} style={{ borderBottom: "1px solid #f1f5f9", background: i % 2 === 0 ? "transparent" : "#f9fafb" }}>
                            <td style={{ padding: "8px 12px", color: "#111827", fontWeight: 600, whiteSpace: "nowrap", position: "sticky", left: 0, background: i % 2 === 0 ? "#ffffff" : "#f9fafb", zIndex: 1 }}>
                              {v.label}
                            ...
                            {nomes.map(eq => {
                              ...
                              return (
                                <td key={eq} style={{ padding: "8px 10px", textAlign: "right", fontVariantNumeric: "tabular-nums", color: isApurar ? "#6b7280" : isCritico ? "#ef4444" : "#111827", fontWeight: 400, fontStyle: isApurar ? "italic" : "normal" }}>
          ...
          return (
            <div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
                {DIAGNOSTICO.map(eq => (
                  <button key={eq.nome} onClick={() => setDiagEquipe(eq.nome)} style={{
                    ...
                    border: `1px solid ${diagEquipe === eq.nome ? (eq.cnesStatus === "expirado" ? "#ef4444" : eq.cnesStatus === "apurar" ? "#6b7280" : "#3b82f6") : "#e4e7ec"}`,
                    background: diagEquipe === eq.nome ? (eq.cnesStatus === "expirado" ? "#450a0a" : eq.cnesStatus === "apurar" ? "#e4e7ec" : "#dbeafe") : "transparent",
                    color: diagEquipe === eq.nome ? (eq.cnesStatus === "expirado" ? "#b91c1c" : eq.cnesStatus === "apurar" ? "#6b7280" : "#1d4ed8") : "#6b7280",
                  }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12, alignItems: "center" }}>
                <span style={{ fontSize: 11, color: "#6b7280" }}>📋 Fonte: SCNES 07/2026 · Exportação DATASUS 20/07/2026</span>
                {(d as any).cnesUbs && (
                  <span style={{ background: "#dbeafe", color: "#1d4ed8", fontSize: 10, fontWeight: 700, padding: "2px 10px", borderRadius: 10, fontFamily: "monospace" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                ...
                <div style={{ background: "#ffffff", borderRadius: 12, padding: 18, border: "1px solid #e4e7ec", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                    <Users size={16} color="#3b82f6" />
                    <span style={{ fontWeight: 700, fontSize: 13, color: "#111827" }}>Composição Mínima da Equipe</span>
                    <span style={{ fontSize: 10, color: "#6b7280" }}>PNAB / Port. Cons. nº 2/2017</span>
                  </div>
                  {[
                    ...
                    return (
                      <div key={p.cargo} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #f1f5f9" }}>
                        <div>
                          <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 2 }}>{p.cargo} · CBO {p.cbo}</div>
                          <div style={{ fontSize: 13, color: "#111827", fontWeight: 500 }}>{p.pessoa.nome}</div>
                          <div style={{ fontSize: 11, color: "#6b7280" }}>{p.pessoa.vinculo}</div>
                        </div>
                  ...
                  <div style={{ marginTop: 10, padding: "8px 0", borderTop: "1px solid #e4e7ec" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 2 }}>ACS (mínimo necessário: {d.acsMin})</div>
                        <div style={{ fontSize: 13, color: "#111827", fontWeight: 500 }}>{d.acs} ACS ativos no SCNES</div>
                      </div>
                ...
                <div style={{ background: "#ffffff", borderRadius: 12, padding: 18, border: "1px solid #e4e7ec", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                    ...
                    <span style={{ fontWeight: 700, fontSize: 13, color: "#111827" }}>População Vinculada</span>
                    <span style={{ fontSize: 10, color: "#6b7280" }}>Port. 3.493/2024</span>
                  </div>

                  <div style={{ textAlign: "center", marginBottom: 16 }}>
                    <div style={{ fontSize: 36, fontWeight: 800, color: popCor }}>{d.populacaoVinculada.toLocaleString("pt-BR")}</div>
                    <div style={{ fontSize: 12, color: "#6b7280" }}>pessoas vinculadas</div>
                  </div>

                  <div style={{ height: 10, background: "#e5e7eb", borderRadius: 5, marginBottom: 8, position: "relative" as const, overflow: "hidden" }}>
                    <div style={{ position: "absolute" as const, height: "100%", width: `${Math.min(100,(d.populacaoRef/d.populacaoMax)*100)}%`, background: "#d1d5db", borderRight: "2px dashed #f59e0b" }} />
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#6b7280", marginBottom: 14 }}>
                    ...
                    <span style={{ color: "#6b7280" }}>Máx: {d.populacaoMax.toLocaleString("pt-BR")}</span>
                  </div>

                  {[
                    { label: "Parâmetro de referência", val: d.populacaoRef.toLocaleString("pt-BR"), cor: "#f59e0b" },
                    { label: "Limite máximo financiamento", val: d.populacaoMax.toLocaleString("pt-BR"), cor: "#6b7280" },
                    ...
                    <div key={item.label} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "4px 0", borderBottom: "1px solid #f1f5f9" }}>
              ...
              {d.pendencias.length > 0 && (
                <div style={{ background: "#ffffff", borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 16, marginBottom: 14, border: "1px solid #b45309" }}>
              ...
              {/* Observação técnica */}
              <div style={{ background: "#f0f7ff", borderRadius: 10, padding: 16, border: "1px solid #dbeafe", marginBottom: 14 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                  <FileText size={14} color="#3b82f6" />
                  <span style={{ fontWeight: 700, fontSize: 12, color: "#3b82f6" }}>Observação Técnica</span>
                </div>
                <p style={{ fontSize: 13, color: "#d1d5db", margin: 0 }}>{d.obs}</p>
              </div>

              {/* Fundamentação legal */}
              <div style={{ background: "#ffffff", borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 16, border: "1px solid #e4e7ec" }}>
                <div style={{ fontWeight: 700, fontSize: 12, color: "#6b7280", marginBottom: 10, textTransform: "uppercase" as const, letterSpacing: 0.5 }}>Fundamentação Legal</div>
                {[
                  ...
                  <div key={item.norm} style={{ padding: "10px 0", borderBottom: "1px solid #f1f5f9" }}>
                    <div style={{ fontWeight: 600, fontSize: 12, color: "#1d4ed8", marginBottom: 4 }}>{item.norm}</div>
          ...
          <div>
            ...
            {periodo === "mensal" && (
              <div style={{ background: "#0c1a2e", border: "1px solid #1d4ed8", borderRadius: 10, padding: 14, marginBottom: 16, display: "flex", gap: 14, alignItems: "flex-start" }}>
                <span style={{ fontSize: 22 }}>📆</span>
                <div>
                  <div style={{ fontWeight: 700, color: "#1d4ed8", marginBottom: 4 }}>Análise Mensal — {new Date().toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</div>
                  <div style={{ fontSize: 12, color: "#1565c0" }}>Competência aberta até ~dia 20 do próximo mês. Verificar no e-Gestor se os indicadores desta competência estão subindo. Focar nos indicadores com maior gap vs meta.</div>
                  <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                    {["✓ Resultados HbA1c/VDRL lançados no PEC?","✓ Produção digitalizada até ontem?","✓ Mutirão de puericultura realizado?","✓ Monitor e-Gestor atualizado?"].map(t => (
                      <span key={t} style={{ background: "#dbeafe", color: "#1d4ed8", padding: "3px 10px", borderRadius: 12, fontSize: 11 }}>{t}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {periodo === "quadrimestral" && (
              <div style={{ background: "#ffffff", border: "1px solid #dbeafe", borderRadius: 14, padding: "16px 20px", marginBottom: 20, display: "flex", gap: 16, alignItems: "flex-start", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                <div style={{ background: "#dbeafe", border: "1px solid #93c5fd", borderRadius: 10, padding: "8px 10px", flexShrink: 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#d1d5db", marginBottom: 10, lineHeight: 1.6 }}>Scores acumulados de maio a agosto. Toda produção lançada até 31/ago contabiliza. Foco em <strong>C2</strong> (pré-natal) e <strong>C6</strong> (puericultura).</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {[
                      ...
                      { t: "Retroativos de gaveta — lançar agora", cor: "#1565c0" },
                    ].map(({ t, cor }) => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12, marginBottom: 28 }}>
              {EQUIPES.map(eq => {
                ...
                const barCor = isApurar ? "#d1d5db" : eq.pts >= 75 ? "#22c55e" : eq.pts >= 60 ? "#f59e0b" : "#ef4444";
                const accentCor = isApurar ? "#d1d5db" : eq.pts >= 75 ? "#16a34a" : eq.pts >= 60 ? "#d97706" : "#dc2626";
                return (
                  <div key={eq.nome} style={{
                    background: "#ffffff", borderRadius: 12, padding: 0, overflow: "hidden", border: "1px solid #e4e7ec",
                    ...
                    <div style={{ padding: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                        <span style={{ fontWeight: 700, fontSize: 13, color: isApurar ? "#6b7280" : "#111827", letterSpacing: 0.2 }}>{eq.nome}</span>
                      ...
                      {isApurar ? (
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
                          ...
                          <div style={{ textAlign: "right" }}>
                            ...
                            <div style={{ fontSize: 10, color: "#d1d5db" }}>para ÓTIMO</div>
                          </div>
                        </div>
                      )}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#d1d5db", marginTop: 5 }}>
            ...
            <div style={{ background: "#ffffff", borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 20, border: "1px solid #e4e7ec", marginBottom: 16 }}>
                  ...
                  <thead>
                    <tr>
                      {["Equipe","Tipo","Pessoas Vinculadas","Referência","Máximo Financ.","Status","Score"].map(h => (
                        <th key={h} style={{ padding: "8px 12px", textAlign: "left", color: "#d1d5db", fontWeight: 700, fontSize: 10, textTransform: "uppercase" as const, letterSpacing: 0.6, borderBottom: "1px solid #e4e7ec" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {DIAGNOSTICO.map((d, idx) => {
                      ...
                      const ptsCor = eq ? (eq.pts >= 75 ? "#22c55e" : eq.pts >= 60 ? "#f59e0b" : "#ef4444") : "#6b7280";
                        ...
                        <tr key={d.nome} style={{ borderBottom: "1px solid #e4e7ec", background: idx % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)" }}>
                          ...
                          <td style={{ padding: "10px 12px" }}>
                            <div style={{ fontWeight: 800, fontSize: 15, color: popCor, marginBottom: 4, fontVariantNumeric: "tabular-nums" }}>
                              {d.populacaoVinculada === 0 ? <span style={{ color: "#d1d5db", fontSize: 12, fontStyle: "italic" }}>a apurar</span> : d.populacaoVinculada.toLocaleString("pt-BR")}
                            </div>
                          ...
                          <td style={{ padding: "10px 12px", color: "#d1d5db", fontVariantNumeric: "tabular-nums" }}>{d.populacaoMax.toLocaleString("pt-BR")}</td>
                          <td style={{ padding: "10px 12px" }}>
                    ...
                    <tr style={{ background: "rgba(245,158,11,0.05)", borderTop: "1px solid rgba(245,158,11,0.15)" }}>
                      <td colSpan={2} style={{ padding: "10px 12px", color: "#6b7280", fontWeight: 700, fontSize: 11, textTransform: "uppercase" as const, letterSpacing: 0.5 }}>Total município</td>
            ...
            {/* Resumo financeiro */}
            <div style={{ background: "#f0fdf4", borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 20, border: "1px solid #bbf7d0" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10 }}>
                {[
                  ...
                  { label: "Pagamento retroativo set/2026", valor: "Score Q2", cor: "#1565c0", icon: "🏦" },
                ].map(item => (
                  <div key={item.label} style={{ background: "#f9fafb", border: `1px solid ${item.cor}20`, borderRadius: 10, padding: "14px 16px", borderLeft: `3px solid ${item.cor}` }}>
        ...
        {aba === "indicadores" && (
          <div>
            {[
              ...
              <div key={item.ind} style={{ background: "#ffffff", borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 16, marginBottom: 12, borderLeft: `4px solid ${item.cor}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  <div>
                    <span style={{ fontWeight: 800, fontSize: 16, color: item.cor }}>{item.ind}</span>
                    <span style={{ fontWeight: 600, fontSize: 13, color: "#111827", marginLeft: 8 }}>{item.desc}</span>
                  </div>
                ...
                <p style={{ fontSize: 13, color: "#d1d5db", marginBottom: 8 }}>{item.texto}</p>
        ...
        {aba === "equipe" && (
          <div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              {EQUIPES.map(eq => (
                <button key={eq.nome} onClick={() => setEquipeAtiva(eq.nome)} style={{
                  ...
                  border: `1px solid ${equipeAtiva === eq.nome ? eq.cor : "#d1d5db"}`,
                  background: equipeAtiva === eq.nome ? eq.cor + "22" : "transparent",
                  color: equipeAtiva === eq.nome ? eq.cor : "#6b7280", cursor: "pointer"
                }}>
                  {eq.nome}
                </button>
              ))}
            ...
            {(() => {
              ...
              return (
                <div>
                  <div style={{ background: "#ffffff", borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 20, marginBottom: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                      <div>
                        <div style={{ fontSize: 20, fontWeight: 800, color: "#111827", marginBottom: 4 }}>{eq.nome}</div>
                        {diag && (
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
                            <span style={{ background: "#f8fafc", border: "1px solid #e4e7ec", color: "#6b7280", fontSize: 11, padding: "2px 8px", borderRadius: 6, fontFamily: "monospace" }}>
                              🏥 {diag.ubs}
                            </span>
                            ...
                            {diag.esb && (
                              <span style={{ background: "#dbeafe", border: "1px solid #0284c7", color: "#7dd3fc", fontSize: 11, padding: "2px 8px", borderRadius: 6 }}>
                                + ESB
                              </span>
                            )}
                            {diag.cnesStatus === "expirado" && (
                              <span style={{ background: "#450a0a", border: "1px solid #ef4444", color: "#b91c1c", fontSize: 11, padding: "2px 8px", borderRadius: 6, fontWeight: 700 }}>
                                🚨 CNES EXPIRADO
                              </span>
                            )}
                            {diag.cnesStatus === "apurar" && (
                              <span style={{ background: "#f8fafc", border: "1px solid #6b7280", color: "#9ca3af", fontSize: 11, padding: "2px 8px", borderRadius: 6, fontWeight: 700 }}>
                                🔍 A APURAR
                              </span>
                            )}
                          </div>
                      ...
                      <div style={{ textAlign: "center", background: "#f8fafc", borderRadius: 10, padding: "12px 20px", flexShrink: 0, border: "1px solid #e4e7ec" }}>
                        <div style={{ fontSize: 28, fontWeight: 800, color: eq.cor }}>+{eq.ganho}</div>
                        <div style={{ fontSize: 11, color: "#6b7280" }}>pontos necessários</div>
                      </div>
                    </div>
                  ...
                  {diag && (
                    <div style={{ background: "#ffffff", borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 18, marginBottom: 16, border: "1px solid #e4e7ec" }}>
                      ...
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 600, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>Composição</div>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8 }}>
                          {[
                            ...
                            <div key={label} style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 12px", border: `1px solid ${dados.cnes === "EXPIRADO" ? "#fca5a5" : dados.cnes === "A APURAR" ? "#d1d5db" : "#dbeafe"}` }}>
                              <div style={{ fontSize: 10, color: "#6b7280", marginBottom: 2 }}>{label}</div>
                              <div style={{ fontSize: 12, fontWeight: 600, color: dados.cnes === "EXPIRADO" ? "#b91c1c" : dados.cnes === "A APURAR" ? "#9ca3af" : "#111827" }}>{dados.nome}</div>
                              <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                                <span style={{ fontSize: 10, color: "#6b7280" }}>CBO {dados.cbo}</span>
                                <span style={{ fontSize: 10, color: dados.vinculo === "Desatualizado" ? "#ef4444" : dados.vinculo === "A apurar" ? "#9ca3af" : "#22c55e" }}>• {dados.vinculo}</span>
                              </div>
                            </div>
                          ))}
                          <div style={{ background: "#f0f7ff", borderRadius: 8, padding: "10px 12px", border: "1px solid #dbeafe" }}>
                            <div style={{ fontSize: 10, color: "#6b7280", marginBottom: 2 }}>ACS</div>
                            <div style={{ fontSize: 12, fontWeight: 600, color: diag.acs >= diag.acsMin ? "#111827" : "#b91c1c" }}>
                      ...
                      {/* População vinculada */}
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 600, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>População Vinculada</div>
                        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 120 }}>
                            <div style={{ height: 6, background: "#d1d5db", borderRadius: 3, overflow: "hidden" }}>
                      ...
                      {diag.pendencias.length > 0 && (
                        <div style={{ marginBottom: 14 }}>
                          <div style={{ fontSize: 11, color: "#6b7280", fontWeight: 600, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>Pendências SCNES</div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            {diag.pendencias.map((p, i) => (
                              <div key={i} style={{ background: "#ffffff", borderRadius: 6, padding: "8px 12px", fontSize: 12, color: p.startsWith("🚨") ? "#b91c1c" : p.startsWith("⚠") ? "#92400e" : "#6b7280", borderLeft: `3px solid ${p.startsWith("🚨") ? "#ef4444" : p.startsWith("⚠") ? "#f59e0b" : "#475569"}` }}>
                                {p}
                              </div>
                            ))}
                          </div>
                      ...
                      {/* Observação */}
                      {diag.obs && (
                        <div style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 12px", fontSize: 12, color: "#6b7280", borderLeft: "3px solid #d1d5db", fontStyle: "italic" }}>
                          {diag.obs}
                        </div>
                      )}
                    </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {inds.map(ind => (
                      <div key={ind.ind} style={{ background: "#f8fafc", borderRadius: 8, padding: 14, display: "flex", gap: 14, alignItems: "flex-start" }}>
                        <div style={{ minWidth: 48, textAlign: "center" }}>
                          <div style={{ fontWeight: 800, fontSize: 15, color: "#f59e0b" }}>{ind.ind}</div>
                          <div style={{ fontSize: 10, color: "#6b7280" }}>{ind.atual}%</div>
                          <div style={{ fontSize: 10, color: "#22c55e" }}>meta {ind.meta}%</div>
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 600, fontSize: 13, color: "#111827", marginBottom: 4 }}>{ind.desc}</div>
                        ...
                        <div style={{ minWidth: 36, textAlign: "center", background: "#22c55e22", borderRadius: 6, padding: "4px 8px" }}>
                          ...
                          <div style={{ fontSize: 9, color: "#6b7280" }}>pts est.</div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {aba === "checklist" && (
          <div>
            {/* Barra progresso */}
            <div style={{ background: "#ffffff", borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 16, marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#111827" }}>Progresso Total do Sprint</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#22c55e" }}>{feitos}/{totalChecks} ({pct}%)</span>
              </div>
              <div style={{ height: 12, background: "#d1d5db", borderRadius: 6, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${pct}%`, background: "linear-gradient(90deg, #22c55e, #16a34a)", borderRadius: 6, transition: "width 0.4s" }} />
              </div>
              <div style={{ fontSize: 11, color: "#6b7280", marginTop: 6 }}>Meta: {totalChecks}/{totalChecks} até 20/agosto</div>
            </div>

            {FRENTES.map(frente => {
              const itens = CHECKLIST.filter(c => c.frente === frente);
              const cor = FRENTE_COR[frente] || "#6b7280";
              return (
                <div key={frente} style={{ background: "#ffffff", borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.07)", padding: 16, marginBottom: 14 }}>
                  ...
                  {itens.map(item => (
                    <div key={item.id} onClick={() => toggle(item.id)} style={{
                      ...
                      <span style={{ fontSize: 13, color: checks[item.id] ? "#6b7280" : "#d1d5db", textDecoration: checks[item.id] ? "line-through" : "none" }}>
                        {item.texto}
                      </span>
                    </div>
                  ))}
        ...
        {aba === "inconsistencias" && (() => {
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "baseline", flexWrap: "wrap", marginBottom: 4 }}>
                    <span style={{ fontWeight: 800, fontSize: 13, color: "#111827" }}>{inc.equipe}</span>
                    <span style={{ fontSize: 12, color: "#6b7280" }}>— {inc.tipo}</span>
                  </div>
                  <p style={{ fontSize: 12.5, color: "#d1d5db", margin: "0 0 8px 0", lineHeight: 1.6 }}>{inc.descricao}</p>
                  <div style={{ background: "rgba(0,0,0,0.06)", borderRadius: 6, padding: "8px 10px" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: 0.5 }}>Ação: </span>
          ...
          return (
            <div>
              ...
              <div style={{ background: "#f8fafc", border: "1px solid rgba(59,130,246,0.3)", borderRadius: 12, padding: "14px 16px", marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: "#1565c0", textTransform: "uppercase", letterSpacing: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    {SIAPS_COMPS.map(c => (
                      <button key={c} onClick={() => setSiapsComp(c)} style={{
                        ...
                        color: siapsComp === c ? "#1d4ed8" : "#6b7280", cursor: "pointer", fontWeight: siapsComp === c ? 700 : 400,
                      }}>{compLabel(c)}</button>
                    ))}
                  </div>
                </div>
                ...
                {siapsLoading && (
                  <div style={{ textAlign: "center", padding: "16px 0", color: "#1565c0", fontSize: 12 }}>⏳ Consultando e-Gestor APS…</div>
                )}
                {siapsError && (
                        ...
                        {siapsData.equipes.map((e, i) => {
                          ...
                          return (
                            <div key={i} style={{ background: g.bg, border: `1px solid ${g.borda}`, borderRadius: 8, padding: "10px 12px", marginBottom: 8, display: "flex", gap: 10, alignItems: "flex-start" }}>
                              <span style={{ fontSize: 10, fontWeight: 800, color: g.cor, background: "rgba(0,0,0,0.08)", padding: "2px 7px", borderRadius: 20, border: `1px solid ${g.borda}`, flexShrink: 0, marginTop: 1 }}>{g.label}</span>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "baseline", marginBottom: 3 }}>
                                  <span style={{ fontWeight: 700, fontSize: 12, color: "#111827" }}>{tipoLabel(e.coTipoEquipe)}</span>
                                  <span style={{ fontSize: 10, color: "#6b7280", fontFamily: "monospace" }}>cód {e.coEquipe}</span>
                                  <span style={{ fontSize: 10, color: g.cor, fontWeight: 600 }}>{e.qtCompetenciasConsecutivas}</span>
                                </div>
                                <div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.5 }}>{e.dsSubTipoSuspensao}</div>
                              </div>
                            </div>
                          );
                        })}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10, marginBottom: 20 }}>
                {[
                  ...
                  { label: "Total", count: INCONSISTENCIAS.length, cor: "#6b7280", bg: "rgba(148,163,184,0.07)", borda: "rgba(148,163,184,0.15)" },
                ].map(s => (
                  <div key={s.label} style={{ background: s.bg, border: `1px solid ${s.borda}`, borderRadius: 10, padding: "12px 16px", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 11, color: "#6b7280", textTransform: "uppercase", letterSpacing: 0.5, marginTop: 2 }}>{s.label}</div>
                  </div>
                ))}
              </div>
