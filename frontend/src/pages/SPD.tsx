...
function AbaTipos({ tipos }: { tipos: any[] | undefined }) {
  if (!tipos) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
...
function AbaReabilitacao({ reab }: { reab: any[] | undefined }) {
  if (!reab) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      {reab.map(s => {
