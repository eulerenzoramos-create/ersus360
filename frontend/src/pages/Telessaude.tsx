...
function AbaEspecialidades({ specs }: { specs: any[] | undefined }) {
  if (!specs) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
...
function AbaESF({ units }: { units: any[] | undefined }) {
  if (!units) return <NaoDisponivelBanner nota="Dados n�o dispon�veis no momento. Integra��o pendente de configura��o no Railway." />;
  return (
    <div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {units.map(u => {
