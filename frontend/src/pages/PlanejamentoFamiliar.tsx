...
function AbaMetodos({ metodos }: { metodos: any[] | undefined }) {
  if (!metodos) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
...
function AbaFaixaEtaria({ faixas }: { faixas: any[] | undefined }) {
  if (!faixas) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 18 }}>
