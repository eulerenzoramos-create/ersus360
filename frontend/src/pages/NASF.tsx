...
function AbaEquipe({ equipe }: { equipe: any[] | undefined }) {
  if (!equipe) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      {equipe.map(p => {
...
function AbaPrioridades({ prioridades }: { prioridades: any[] | undefined }) {
  if (!prioridades) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
