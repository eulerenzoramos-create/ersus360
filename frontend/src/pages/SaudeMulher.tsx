...
function AbaDashboard({ dash }: { dash: any }) {
  if (!dash) return <NaoDisponivelBanner nota="Dados indisponÃ­veis. IntegraÃ§Ã£o nÃ£o configurada no Railway. Nenhum valor foi inventado." />;
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 22 }}>
        ...
        <KpiCard label="Sem HIV/SÃ­filis"        value={dash.sem_teste_hiv_sif}         sub="exames pendentes"          cor={dash.sem_teste_hiv_sif>0?"#d97706":"#16a34a"} icon={<AlertTriangle size={14} color={dash.sem_teste_hiv_sif>0?"#d97706":"#16a34a"}/>}/>
        <KpiCard label="Indicadores crÃ­ticos"   value={dash.indicadores_criticos}      sub="abaixo da meta"            cor={dash.indicadores_criticos>0?"#dc2626":"#16a34a"} icon={<AlertTriangle size={14} color={dash.indicadores_criticos>0?"#dc2626":"#16a34a"}/>}/>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Gestantes e consultas â€” 6 meses</div>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Partos â€” normal vs total</div>
          <div style={{ height: 190 }}>
            <ResponsiveContainer width="100%" height="100%">
        ...
        return (
          <div key={nivel} style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: cor, marginBottom: 10, textTransform: "uppercase" as const, letterSpacing: 1 }}>
              {nivel === "critico" ? "CrÃ­tico â€” abaixo da meta" : nivel === "atencao" ? "AtenÃ§Ã£o" : "OK"}
            </div>
            {grupo.map(ind => {
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ background: "#ec4899", color: "#fff" }}>
              ...
              <th style={{ padding: "9px 10px", textAlign: "center" }}>HIV/SÃ­f.</th>
          ...
          <tbody>
            {lista.map((g, i) => (
              <tr key={g.id} style={{ borderTop: "1px solid #f3f4f6", background: g.risco === "alto" ? "#fff7f7" : i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                ...
                <td style={{ padding: "9px 10px", textAlign: "center" }}>{g.odonto ? <span style={{ color: "#16a34a" }}>âœ“</span> : <span style={{ color: "#dc2626" }}>âœ—</span>}</td>
                <td style={{ padding: "9px 10px", textAlign: "center" }}>{g.hiv_sifilis ? <span style={{ color: "#16a34a" }}>âœ“</span> : <span style={{ color: "#dc2626" }}>âœ—</span>}</td>
    ...
    <div>
      {puerperas.filter(p => !p.consulta_puerp).length > 0 && (
        <div style={{ background: "#fff7f7", border: "1px solid #fca5a5", borderRadius: 8, padding: "10px 16px", marginBottom: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#dc2626", marginBottom: 6 }}>PuÃ©rperas sem consulta puerperal</div>
          {puerperas.filter(p => !p.consulta_puerp).map(p => (
            <div key={p.id} style={{ fontSize: 12, color: "#7f1d1d", padding: "2px 0" }}>â€¢ {p.iniciais} â€” {p.equipe} (parto {p.data_parto}, {p.dias_puerp} dias)</div>
          ))}
        </div>
      )}
        ...
        {puerperas.map(p => (
          <div key={p.id} style={{ background: "#fff", border: `1px solid ${p.consulta_puerp ? "#d1fae5" : "#fca5a5"}`, borderRadius: 8, padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              {p.consulta_puerp
                ? <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 700 }}>âœ“ Consultada</span>
                : <span style={{ fontSize: 11, color: "#dc2626", fontWeight: 700 }}>âš  Pendente</span>}
            </div>
            <div style={{ fontSize: 12, color: "#6b7280" }}>{p.equipe}</div>
            <div style={{ fontSize: 12, color: "#374151", marginTop: 4 }}>Parto: {p.data_parto} Â· {p.dias_puerp} dias</div>
    ...
    <div style={{ padding: "0 0 32px", fontFamily: "system-ui,sans-serif" }}>
      <div style={{ background: "linear-gradient(135deg,#1565c0 0%,#1351b4 100%)", color: "#fff", padding: "20px 24px 16px", borderRadius: "0 0 16px 16px", marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>SaÃºde da Mulher</h1>
            <p style={{ fontSize: 13, opacity: .85, margin: 0 }}>PrÃ©-natal Â· PuerpÃ©rio Â· Preventivo Â· SISPRENATAL Â· FMS ApuÃ­/AM</p>
          </div>
          {dash && (
      ...
      <div style={{ padding: "0 24px" }}>
        ...
        {aba === "dashboard" && !dash && <NaoDisponivelBanner nota="IntegraÃ§Ã£o com sistema externo ainda nÃ£o configurada no Railway. Nenhum valor foi inventado." />}
