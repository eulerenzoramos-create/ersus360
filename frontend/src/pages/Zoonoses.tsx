...
function AbaAcidentes({ acidentes }: { acidentes: any[] | undefined }) {
  if (!acidentes) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
      {acidentes.map(a => {
...
function AbaRaiva({ raiva }: { raiva: any | undefined }) {
  if (!raiva) return <NaoDisponivelBanner nota="Dados nao disponiveis — integracao pendente de configuracao no Railway." />;
  return (
    <div>
