"""Router: /api/cadeia-frio-apui — ERSUS 360 — CNES + SI-PNI dados abertos"""
...
_NOTA = "Temperatura/câmaras individuais requerem sistema local (pendente). CNES/PNI como proxy de estrutura vacinal."
@router.get("/dashboard")
async def dashboard(ano: int = Query(0)):
    if not ano: ano = _ANO()
    cnes = await resumo_estabelecimentos(); pni = await buscar_cobertura(ano)
